...
const calculateCourierCharge = async (weight) => {
  ...
  return rows[0]?.charge || 20; // Default to highest charge if not found
};
  ...
  for (const pkg of optimizedPackages) {
    if (pkg.items.length > 0) {
      result.push({
        ...
        courierPrice: await calculateCourierCharge(pkg.totalWeight)
      });
    }
  }
...
app.post('/api/v1/orders/calculate', async (req, res) => {
  try {
    ...
    await dbPool.query(
      ...
      [totalPrice, totalWeight, packages.length, totalCourierCharge]
    );
